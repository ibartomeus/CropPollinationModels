...
unique(field_data$study_id2)	# 144 studies, as before
...
nrow(field_data2a)	# 2993 + 16 new sites for Jamie_Ellis_Citrullus_lanatus_USA_2014

#setdiff(unique(field_data2a$study_id2), unique(field_data2a$study_id2))
...
# Create field level data summary
field_data_summary_edited <- 
  unique( field_data[,c("study_id2","taxa_recorded","use_visits_or_abundance")] )
field_data_summary_edited 
...
for (i in 1:length(studies)) {
  ...
  # visit rate studies
  if (subset(field_data_summary_edited, study_id2==studies[i])$use_visits_or_abundance == "use visits") {
