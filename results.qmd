...
modelout_mod9 = modelout_mod9[order(modelout_mod9$mean_wi), , drop = FALSE]
...
modelout_mod9 = modelout_mod9[order(modelout_mod9$mean_hb), , drop = FALSE]
...
timepoints = unique(estimates$timepoint)
timepoints

estimates_means = data.frame()

for (i in length(timepoints):1) {
	this_time = subset(estimates, timepoint==timepoints[i])
		
		estimate_HB = mean(this_time$estimate_HB, na.rm=T)
		estimate_WI = mean(this_time$estimate_WI, na.rm=T)
		estimate_all_insects = mean(this_time$estimate_all_insects, na.rm=T)
		estimate_richness = mean(this_time$estimate_richness, na.rm=T)

		if (estimate_HB=="NaN") { estimate_HB = NA }
		if (estimate_WI=="NaN") { estimate_WI = NA }
		if (estimate_all_insects=="NaN") { estimate_all_insects = NA }
		if (estimate_richness=="NaN") { estimate_richness = NA }		
	
		this_means = data.frame(timepoint = timepoints[i],estimate_HB,estimate_WI,estimate_all_insects,estimate_richness)
		estimates_means = rbind(this_means,estimates_means)
	
}

#estimates_means

estimates_means$index = 1:nrow(estimates_means)
estimates = merge(estimates, estimates_means[,c("index","timepoint")], by="timepoint")

plot(1:nrow(estimates_means),estimates_means$estimate_HB, col = "white", las = 1, xlab = "", ylab = "estimates", ylim = c(-0.1, 0.4),  xaxt = "n")
axis(1, at = 1:nrow(estimates_means), labels = estimates_means$timepoint, las=2, cex.axis = 0.7)

points(which(!is.na(estimates_means$estimate_HB)),subset(estimates_means, !is.na(estimate_HB))$estimate_HB, t = "l", col = "#00bb5c", lwd=2)
points(which(!is.na(estimates_means$estimate_WI)),subset(estimates_means, !is.na(estimate_WI))$estimate_WI, t = "l", col = "#c96d00", lwd=2)
points(which(!is.na(estimates_means$estimate_all_insects)),subset(estimates_means, !is.na(estimate_all_insects))$estimate_all_insects, t = "l", col = "#ff0000", lwd=2)
points(which(!is.na(estimates_means$estimate_richness)),subset(estimates_means, !is.na(estimate_richness))$estimate_richness, t = "l", col = "#00a2ca", lwd=2)

points(subset(estimates, !is.na(estimate_HB))$index,subset(estimates, !is.na(estimate_HB))$estimate_HB, t = "p", col = "#00bb5c", pch=19, cex=1.5)
points(subset(estimates, !is.na(estimate_WI))$index,subset(estimates, !is.na(estimate_WI))$estimate_WI, t = "p", col = "#c96d00", pch=19, cex=1.5)
points(subset(estimates, !is.na(estimate_all_insects))$index,subset(estimates, !is.na(estimate_all_insects))$estimate_all_insects, t = "p", col = "#ff0000", pch=19, cex=1.5)
points(subset(estimates, !is.na(estimate_richness))$index,subset(estimates, !is.na(estimate_richness))$estimate_richness, t = "p", col = "#00a2ca", pch=19, cex=1.5)

#legend("topright", inset=.03, legend=c("honey bee", "wild insects", "all insects", "richness"), col=c("#00bb5c", "#c96d00", "#ff0000", "#00a2ca"), lty=1, lwd=2, box.lty=0)
legend("topright", inset=.03, legend=c("all insects", "wild insects", "richness", "honey bee"), col=c("#ff0000", "#c96d00", "#00a2ca", "#00bb5c"), lty=1, lwd=2, box.lty=0)
